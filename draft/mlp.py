...
class Sigmoidal2LayerMLP_WithSoftmax:
    ...
    def __init__(self,
        ...
        self.Wih = 2 * mul_wih * np.random.rand(input_size,
                                                hidden_size) - mul_wih  # input to hidden weights
        ...
        self.Who = 2 * mul_who * np.random.rand(hidden_size,
                                                output_size) - mul_who  # hidden to output weights
    ...
    def loss(self, yi):
        return -np.log(self.output_act[0, yi]) + self.l2 * (
            np.sum(self.Who ** 2) + np.sum(self.Wih ** 2))
    ...
    def backward(self, yi):
        ...
        self.hidden_preact_grad[:] = self.hidden_posact_grad * (
            self.hidden_act * (1 - self.hidden_act))
    ...
    def train(self, X, y, batch_size=100, max_epochs=100, Xv=None, yv=None):
        ...
        if d != self.input_size():
            raise ValueError(
                'X should have the number of column equal to the input_size.')
        ...
        for epoch in xrange(max_epochs):
            ...
            for i, batch_inds in enumerate(batches_inds):
                ...
                Wih_momemtum = self.momen_decay * Wih_momemtum - self.lr * (
                    self.Wih_grad / batch_nsamples)
                hb_momentum = self.momen_decay * hb_momentum - self.lr * (
                    self.hb_grad / batch_nsamples)
                Who_momentum = self.momen_decay * Who_momentum - self.lr * (
                    self.Who_grad / batch_nsamples)
                ob_momentum = self.momen_decay * ob_momentum - self.lr * (
                    self.ob_grad / batch_nsamples)
            ...
            print "\nepoch = %d | val accu = %.3f | loss = %.3f | |W**2| = \
            %.3f | lr = %.5f | momen_decay = %.3f | l2 = %.5f" % (
                epoch, self.validate(), loss,
                np.sum(self.Wih ** 2) + np.sum(self.Who ** 2), self.lr,
                self.momen_decay, self.l2)
