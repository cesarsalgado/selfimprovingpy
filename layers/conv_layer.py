...
def convolve_multi_channel_plus_bias(input_vol, output, kernel, bias, stride):
    ...
    for ki in xrange(0, input_vol.shape[1] - kh + 1, stride):
        for kj in xrange(0, input_vol.shape[2] - kw + 1, stride):
            output[oi, oj] = (
                np.sum(input_vol[:, ki:ki + kh, kj:kj + kw] * kernel) + bias)
            oj += 1
        oi += 1
...
class ConvLayer(WeightLayer):
    ...
    def __init__(self, filter_size, stride, pad, n_filters, l1, l2,
                 init_weights_info):
    ...
    def init_weights(self, rand_state, input_channels):
        if self.init_weights_info['type'] == 'gaussian':
            self.weights = self.init_weights_info['std'] * \
                rand_state.randn(self.n_filters, input_channels,
                                 self.filter_size, self.filter_size)
            self.biases = np.full((self.nfilters, 1),
                                  self.init_weights_info['bias'])
        else:
    ...
    def setup(self, rand_state, prev_layer):
        ...
        if pad > 0:
            ...
            self.padded_input = np.zeros((
                batch_size, input_channels, input_h + 2 * pad, input_w + 2 * pad
            ))
            self.padded_input_grad = np.zeros((
                batch_size, input_channels, input_h + 2 * pad, input_w + 2 * pad
            ))
            self.input_ = self.padded_input[
                :, :, pad:pad + input_h, pad:pad + input_w]
            self.input_grad = self.padded_input_grad[
                :, :, pad:pad + input_h, pad:pad + input_w]
        ...
        output_h = calc_output_size(
            input_h, self.filter_size, self.pad, self.stride)
        output_w = calc_output_size(
            input_w, self.filter_size, self.pad, self.stride)
        if not output_h.is_integer() or not output_w.is_integer():
            raise ValueError(
                "ConvLayer filters configuration doesn't fit nicely in input \
                dimensions.")
    ...
    def forward(self):
        ...
        for b in xrange(batch_size):
            for f in xrange(self.n_filters):
                convolve_multi_channel_plus_bias(
                    self.padded_input[b], self.output[b, f], self.weights[f],
                    self.biases[f], self.stride)

    def backward(self):
        batch_size = self.input_.shape[0]
        for b in xrange(self.input_.shape[0]):
            for f in xrange(self.n_filters):
                ...
                kh, kw = kernel.shape[1:]
                oi, oj = 0, 0
                for ki in xrange(0, input_vol.shape[1] - kh + 1, self.stride):
                    for kj in xrange(0, input_vol.shape[2] - kw + 1,
                                     self.stride):
                        top_grad = self.output_grad[b, f, oi, oj]
                        input_grad_vol[:, ki:ki + kh, kj:kj + kw] += (
                            kernel * top_grad)
                        self.weights_grad[f] += (
                            input_vol[:, ki:ki + kh, kj:kj + kw] * top_grad)
                        self.biases_grad[f] += top_grad
                        kj += 1
                    oi += 1
        for f in xrange(self.n_filters):
            self.weights_grad[f] = self.weights_grad[f]/float(batch_size) + \
                self.l2 * self.weights[f] + self.l1 * np.sign(self.weights[f])

# keeping this imcomplete method just in case I need parts of it in the future.


def convolve_multi_channel_with_stride1_and_right_padding(input_vol, kernel,
                                                          pad):
