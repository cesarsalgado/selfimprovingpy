...
class FullyConnectedLayer(WeightLayer):
    def __init__(self, output_size, lr, l1, l2, init_weights_info):
    ...
    def backward(self):
        ...
        for b in xrange(batch_size):
            ...
            self.weights_grad += np.outer(self.input_[b], self.output_grad[b]) + 2*self.l2*self.weights + self.l1*np.sign(self.weights)
